...
function authHeaders() {
  return {
    ...
    // Example auth: adjust if your backend expects it differently (e.g., x-api-key)
    Authorization: Bearer ${API_ID},
    // Or if your API uses a header like:
    // "x-api-key": API_ID,
  };
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function loadVillages(state, district) {
    ...
    try {
      const data = await fetchJson(
        ${BASE_URL}/api/villages?state=${encodeURIComponent(
          state
  ...
  async function fetchWeather(state, district, village) {
    try {
      const payload = await fetchJson(
        ${BASE_URL}/api/weather?state=${encodeURIComponent(
          state
